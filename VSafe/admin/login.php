...
<?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        // 02. Check Whether the inserted username and password exist in the Admin table if yes redirect to the admin dashboard
        $sqlAdminCheck = "SELECT * FROM tbl_admin WHERE user_name='$username' AND password='$password'";
        $resAdminCheck = mysqli_query($conn, $sqlAdminCheck);
        $AdminCount = mysqli_num_rows($resAdminCheck);
        // 03. There is a existing user in the admin table
        if($AdminCount > 0)
        {
            $_SESSION['admin-user'] = $username;
            $_SESSION['pwrd'] = $password;
            header('location:'.SITEURL.'admin-dashboard.php');
        }else{  // 04. There isn't a user in the admin table
